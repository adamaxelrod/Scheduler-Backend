...
class Crew(object):
    ...
    def hadRecentPrimetime(self, week, threshold):
        if (int(week) > 1):
            lowerBound = int(week) - \
                threshold if (int(week) - threshold > 0) else 1
            upperBound = int(week)
    ...
    def addOffWeekToSchedule(self, week):
        self.getSchedule()[str(week)] = Game.Game(
            week, Constants.WEEK_OFF, Constants.WEEK_OFF)
        self.totalOff += 1
    ...
    def isDueForOff(self, week):
        ...
        if (self.getTotalOff() >= Constants.MAX_OFF_WEEKS):
            return False
        for prev in range(intWeek-Constants.OFF_THRESHOLD, intWeek):
            if (self.isAssigned(str(prev)) == False):
                return False
        if (self.isAssigned(str(intWeek-1)) == True):
            if (intWeek <= 8):
                return True if self.getTotalOff() == 0 else False
            elif (intWeek <= 17):
                return True if self.getTotalOff() < Constants.MAX_OFF_WEEKS else False
            else:
                return False
        return False
    ...
    def hadOffWeekRecently(self, week):
        if (int(week) > 1):
            lowerBound = int(
                week) - Constants.OFF_THRESHOLD if (int(week) - Constants.OFF_THRESHOLD > 0) else 1
            upperBound = int(week)
            for prev in range(lowerBound, upperBound):
                if (self.schedule[str(prev)].getAway() == Constants.WEEK_OFF):
                    return True
        return False
    ...
    """ Get ranking value for this crew for this game """

    def getRanking(self, week, home, away, primetime=None):
        ranking = self.getDetailedRanking(week, home, away, primetime)
    ...
    def getDetailedRanking(self, week, home, away, primetime=None):
        ...
        if (primetime != None):
            ...
            hadRecentPrimetime = self.hadRecentPrimetime(
                week, Constants.RECENT_PRIMETIME_THRESHOLD)
        else:
            primetimeAllowed = True
            primetimeCount = 0
            extraPrimetimeAllowed = True
            hadRecentPrimetime = False

        if (self.isAssigned(week) != True):
            hadTeamsAtAll = self.hadAtLeastOneTeam(home, away)
            hadOnlyOneTeam = self.hadOnlyOneTeam(home, away)
            hadTeamsLastTwoWeeks = self.hadTeamsRecently(
                week, home, away, Constants.TEAM_RECENTLY_THRESHOLD)
            hadTeamWithinFourWeeks = self.hadTeamsRecently(
                week, home, away, Constants.TEAM_RECENTLY_FULL_THRESHOLD)
